...
library(readr)
pairfam <- read_csv("~/Desktop/r-project/Multivariate Analysis/pairfam_synth_data.csv")
head(pairfam, 14)
summary(pairfam)

data <- na.omit(pairfam)
summary(data)
```
...
# I create a finction to make a figure of a certain size
...
for (i in seq(1,ncol(data3))){
    hist(data3[,i],main = '',xlab = colnames(data)[i])
}
...
par(mfrow = c(3,3))
for (i in seq(1,ncol(data3))){
    qqPlot(as.numeric(data3[,i]),ylab=colnames(data3)[i])
}
```
...
```{r}
normal_uni_test <- data.frame(matrix(0,ncol(data3),3))
rownames(normal_uni_test) <- colnames(data3)
colnames(normal_uni_test) <- c('Statistic','p-value','Normal')

alpha <- 0.05
for (i in seq(1,ncol(data3))){
    normal_uni_test[i,1] <- shapiro.test(as.numeric(data3[,i]))$statistic
    normal_uni_test[i,2] <- shapiro.test(as.numeric(data3[,i]))$p.value
...
cortest.bartlett(data3)
#bart.test <- cortest.bartlett(r, n = nrow(data3))
...
```{r}
fa.parallel(data3,fm='fa',ylabel='Eigenvalues')
...
library(stats)
fit <- factanal(data3, Nfacs, rotation="varimax")
...
factor_analysis <- fa(data3,nfactors=2,fm='ml',rotate='varimax',max.iter=100)
factor_analysis
...
```{r}
data4 <- data3[,c(5, 6:9)]
factor_analysis <- fa(data4,nfactors=2,fm='ml',rotate='varimax',max.iter=100)
factor_analysis
```
